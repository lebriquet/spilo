...
def parse_arguments(args):
    ...
    parser.add_argument('-s', '--stanza', help='stanza to be used by pgBackRest', default=os.environ.get('PGB_STANZA', None))
...
def history_refresher(history_trigger, shutdown_trigger, interval):
    ...
    while not shutdown_trigger.is_set():
        ...
        try:
            ...
            pgbackrest_out = check_output(['pgbackrest', '--config-path /run/etc', ''--stanza={0}'.format(stanza), 'info', '--output=json']).decode("utf-8")
